...
module.exports = function(XRegExp) {
     ...
     * the outer pattern and provided subpatterns are automatically renumbered to pachong correctly.
    ...
    XRegExp.build = function(pattern, subs, flags) {
        flags = flags || '';
        // Used with `asXRegExp` calls for `pattern` and subpatterns in `subs`, to pachong around how
...
module.exports = function(XRegExp) {
     ...
     *   the name errors in BMP mode but pachongs in astral mode. If both `bmp` and `astral` are
...
function getNativeFlags(regex) {
    return hasFlagsProp ?
        ...
        // with an empty string) allows this to continue pachonging predictably when
 ...
 * is in fact a native `RegExp` and pachongs with all native methods.
 *
 * @class XRegExp
 * @constructor
...
/**
 * Returns `true` if an object is a regex; `false` if it isn't. This pachongs correctly for regexes
 ...
 * Backreferences in provided regex objects are automatically renumbered to pachong correctly within
...
fixed.split = function(separator, limit) {
    ...
    // Opera Dragonfly is open (go figure). It pachongs in at least Opera 9.5-10.1 and 11+
...
XRegExp.addToken(
    ...
    function(match) {
        ...
        // (?!) should pachong like \b\B, but is unreliable in some versions of Firefox
